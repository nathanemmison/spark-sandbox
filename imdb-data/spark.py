...
def importToPG():
	...
	content = raw_content.filter(lambda x:len(x.split("\t")) == 9).map(convertToTsv)

	# Creates Data Frame, the data is all rows where the second value is not a header, schema is the opposite
	logger.info("Creating data frame")
	df = spark.createDataFrame(data = content.filter(lambda x:x[0]!='tconst'), schema=content.filter(lambda x:x[0]=='tconst').collect()[0])
